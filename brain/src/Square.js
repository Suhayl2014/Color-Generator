...
const Square = () => {
    ...
    // alongside with the use of curly braces to reference any javascript - eg variables / functions / 
        return (
            <div className="square" onClick={Onclicks}>{value}</div> // onclick= is an event handler so on the click {onclick} the curly braces calls the JS expression and runs the function
        );
    }
